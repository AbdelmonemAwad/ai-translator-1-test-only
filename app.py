...
def settings_ai(subsection='models'):
    ...
    current_settings = {}
    for setting in Settings.query.filter(Settings.section.in_(['MODELS', 'API'])).all():
        current_settings[setting.key] = setting.value
...
def settings_paths():
    ...
    current_settings = {}
    for setting in Settings.query.filter(Settings.section == 'PATHS').all():
        current_settings[setting.key] = setting.value
...
def settings_media(subsection='overview'):
    ...
    # Define sub-tabs for Media section
    sub_tabs = [
        {'key': 'overview', 'label': 'overview', 'icon': 'grid', 'url': url_for('settings_media', subsection='overview')},
        {'key': 'plex', 'label': 'plex_server', 'icon': 'tv', 'url': url_for('settings_media', subsection='plex')},
        {'key': 'jellyfin', 'label': 'jellyfin_server', 'icon': 'film', 'url': url_for('settings_media', subsection='jellyfin')},
        {'key': 'radarr', 'label': 'radarr_movies', 'icon': 'video', 'url': url_for('settings_media', subsection='radarr')},
        {'key': 'sonarr', 'label': 'sonarr_series', 'icon': 'play', 'url': url_for('settings_media', subsection='sonarr')}
    ]
    
    current_settings = {}
    media_sections = ['PLEX', 'JELLYFIN', 'EMBY', 'KODI', 'RADARR', 'SONARR']
...
def download_github_release():
    ...
    release_files = [
        'ai-translator-v2.2.5-github-complete.zip',   # Latest with installation files
